...
def lambda_handler(event, context):
    ...
    GET    /profile/favorites          → Ver recetas favoritas
    POST   /profile/favorites          → Guardar receta favorita
    DELETE /profile/favorites/{recipe_id} → Eliminar receta favorita
    """
    try:
        ...
        with conn:
            with conn.cursor() as cursor:
                ...
                if http_method == "GET" and "/favorites" not in path:
                    cursor.execute(
                        """SELECT up.avatar_emoji, up.bio, up.favorite_cuisine, u.email, u.username
                    ...
                    if not profile:
                        ...
                        # DictCursor devuelve diccionarios, no tuplas
                        profile_data = {
                    ...
                    return _response(200, profile_data)
                ...
                # PUT /profile
                    ...
                    cursor.execute(
                        """INSERT INTO user_profiles (user_id, avatar_emoji, bio, favorite_cuisine)
                           ...
                           avatar_emoji = %s, bio = %s, favorite_cuisine = %s""",
                        (user_id, avatar_emoji, bio, favorite_cuisine, avatar_emoji, bio, favorite_cuisine)
                    )
                    conn.commit()
                ...
                # GET /profile/favorites
                    ...
                    favorites = [
                        {"id": f["id"], "title": f["title"], "ingredients": f.get("ingredients", ""), 
                         "steps": f.get("steps", ""), "tip": f.get("tip", ""), 
                         "prep_time": f.get("prep_time", ""), "created_at": str(f["created_at"])}
                ...
                # POST /profile/favorites
                    ...
                    cursor.execute(
                        """INSERT INTO favorite_recipes (user_id, title, ingredients, steps, tip, prep_time)
                           VALUES (%s, %s, %s, %s, %s, %s)""",
                    ...
                    return _response(201, {
                        "success": True,
                        "recipe_id": recipe_id,
                        "message": "Receta guardada a favoritas"
                    })
